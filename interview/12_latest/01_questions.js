...
let first = second; // tries to evaluate second, but second is not defined yet
let second = 1;

/* the call to add(undefined, 1) throws an error because
second is not defined when first is initialized. The initialization
of first is evaluated before the initialization of second, so the
reference to second in the default value for first throws a ReferenceError. */
...
var person = new Person("Nicholas");
var notAPerson = Person.call(person, "Michael"); // error! not new
